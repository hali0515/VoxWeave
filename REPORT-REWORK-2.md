...
| SWEEP-6 | Fixed by associating each selected window with its originating clean run. The exact three 2.5-second utterances separated by 0.5 seconds now all survive selection. |
| SWEEP-7 | Fixed as a documentation defect. The fill-pass comment now states that it may select another usable window and that the same-run gap remains enforced; `test_select_snippets_fill_can_reuse_one_clean_run_with_a_real_gap` pins the reported `(100,106)` / `(107,113)` behavior. |
