...
def _strict_failure(value: object) -> dict[str, Any] | None:
    if value is None:
        return None
    return {
...
def _denied_charge(value: object) -> dict[str, Any] | None:
    if value is None:
        return None
    return {
...
def _work_value(work: AuthorityJobWorkReceipt) -> dict[str, Any]:
    mismatch = getattr(work, "route_mismatch")
    job_limits = getattr(work, "limits")
    totals = getattr(work, "totals")
    return {
        "status": getattr(work, "status"),
        "route_status": getattr(work, "route_status"),
        "route_mismatch": (
            None
            if mismatch is None
            else {
        ...
        "route_claims": [
            ...
            for claim in getattr(work, "route_claims")
        ],
        ...
        "limits": {
            "call_limit": job_limits.call_limit,
            "state_limit": job_limits.state_limit,
            "edge_limit": job_limits.edge_limit,
            "interval_limit": job_limits.interval_limit,
            "normalize_char_limit": job_limits.normalize_char_limit,
        },
        "totals": {
            "states": totals.states,
            "edges": totals.edges,
            "intervals": totals.intervals,
            "normalize_chars": totals.normalize_chars,
        },
        "terminal_call_index": work.terminal_call_index,
        "denied_charge": _denied_charge(work.denied_charge),
        "calls": [
            {
                ...
                "strict_failure": _strict_failure(row.strict_failure),
                "limits": {
                ...
                "allocator": _lane(row.allocator),
                "verifier": None if row.verifier is None else _lane(row.verifier),
            }
...
def _context_value(context: IssuedAlignContext) -> dict[str, Any]:
    thawed = thaw_json(_align_context_stable_fields(context))
...
def _history_value(
    context: IssuedAlignContext, stable: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    try:
        policy_bits = {
            name: struct.pack(">d", policy[name]).hex() for name in _POLICY_KEYS
        }
    except (KeyError, TypeError, ValueError, struct.error) as exc:
        raise _binding_failure() from exc
    return {
        ...
        "registry_family": context.engine_family,
        ...
        "authority_limit_profile_kind": getattr(profile, "kind", None),
        "authority_limit_profile_digest": getattr(profile, "profile_digest", None),
...
def _route_value(
    ...
    physical_calls: Sequence[EvidenceCorePhysicalCall],
    work: AuthorityJobWorkReceipt,
) -> dict[str, Any]:
    blocks = stable.get("blocks")
    if not isinstance(blocks, list):
        raise _binding_failure()
    calls_by_source: dict[int, int] = {}
    for call in physical_calls:
        for source in call.source_block_indices:
            calls_by_source.setdefault(source, call.call_index)
    skips_by_source = {row.source_index: row for row in work.skipped_blocks}
    physical_by_index = {call.call_index: call for call in physical_calls}
    ...
    for delivery_index, block in enumerate(blocks):
        ...
        skip = skips_by_source.get(source) if type(source) is int else None
        if context.route_kind in ("ctc-full", "mms-full"):
        ...
        if context.route_kind == "qwen-crop" and call_index is not None:
            physical = physical_by_index[call_index]
    ...
    return {
        "digest": _digest_projection("p6-route-plan-v1", entries),
        "entries": entries,
    }


def _physical_values(core: EvidenceCore) -> list[dict[str, Any]]:
    return [
        {
            "call_index": row.call_index,
            "source_block_indices": list(row.source_block_indices),
        ...
        for row in core.physical_calls
    ]


def _legacy_value(
    receipts: Sequence[LegacyCallDistributionReceipt],
) -> dict[str, Any]:
    calls = [
        {
            "call_index": index,
    ...
    return {
        "digest": _digest_projection("p6-legacy-distribution-v1", calls),
        "calls": calls,
    }


def _authority_value(
    distribution: AuthorityDistributionReceipt, core: EvidenceCore
) -> dict[str, Any]:
    valid = distribution.status == "valid"
    work = _work_value(distribution.work)
    digest_input = {
        ...
        "owner_unit_ids": (
            [list(owner) for owner in distribution.owners or ()] if valid else None
        ),
        "consumed_count": distribution.consumed_count,
        "leftover_unit_ids": list(distribution.leftovers),
        "reasons": list(core.authority_reasons),
        "work": work,
    }
    return {
        "status": distribution.status,
        "digest": _digest_projection("p6-authority-distribution-v1", digest_input),
        **{key: value for key, value in digest_input.items() if key != "status"},
    }


def _legacy_ids_by_source(
    receipts: Sequence[LegacyCallDistributionReceipt],
) -> dict[int, list[str]]:
    projected: dict[int, list[str]] = {}
    for receipt in receipts:
        for source, unit_ids in zip(
            receipt.owner_source_indices, receipt.owner_unit_ids, strict=True
        ):
            if source in projected:
                raise _binding_failure()
            projected[source] = list(unit_ids)
    return projected


def _block_values(
    core: EvidenceCore, legacy_ids: Mapping[int, list[str]]
) -> list[dict[str, Any]]:
    return [
        {
            "source_index": block.source_index,
            "legacy_unit_ids": list(legacy_ids.get(block.source_index, [])),
            "authority_unit_ids": (
                None
