...
from voxweave.core.boundary_task import (
    BoundaryTask,
    _canonical_language,
    _char_count,
    _clean_indices as _clean_indices,
    _segment_char_counts as _segment_char_counts,
    _validate_selection,
)
...
# The boundary task itself is model-free and now lives in the core, where the
# deterministic optimizer shares it; this module keeps the historical name for
# the same class object, so equality and ``isinstance`` are unaffected.
SemanticBreakRequest = BoundaryTask
...
# boundary indices; a decision contains only validated indices.  ``BoundaryTask``
# is imported from :mod:`voxweave.core.boundary_task` at the top of this module.
