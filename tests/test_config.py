...
def test_default_template_omits_retired_semantic_mode(conf_at):
    config.ensure_default_config()
    text = conf_at.read_text(encoding="utf-8")
    assert "semantic_split" not in text
    assert "[semantic]" not in text
    assert "VOXWEAVE_SEMANTIC" not in text


def test_semantic_config_section_is_no_longer_recognized(conf_at, caplog):
    conf_at.write_text('[semantic]\nmodel = "local/custom"\n', encoding="utf-8")
    with caplog.at_level(logging.WARNING, logger="voxweave"):
        config._load()
    assert any("unknown config key 'semantic'" in row.message for row in caplog.records)
